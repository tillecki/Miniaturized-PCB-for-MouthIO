...
uint8_t MC36XX::readRegister8(uint8_t reg)
{
    ...
    //Wire.requestFrom(MC36XX_CFG_I2C_ADDR, 1, true);
    value = Wire.read();
...
bool MC36XX::start(void)
{
    ...
    // Initialize I2C
    delay(2000);
    Wire.setPins(MC36XX_CFG_BUS_I2C_SDA, MC36XX_CFG_BUS_I2C_SCL);
    Wire.begin();
    Serial.println("Begin wire with MC36XX...");
#else
    //Set active-low CS low to start the SPI cycle
    digitalWrite(chipSelectPin, HIGH);
...
//Running mc36xx in high speed SPI (8MHz)
    ...
    Serial.println("MC36XX detected and successfully initialized!");
    return true;
}
